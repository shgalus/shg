...
BOOST_DATA_TEST_CASE(distribution_test,
     ...
     SHG::Laplace_distribution d(mu, lambda);
     BOOST_CHECK(std::abs(p - d.cdf(d.invcdf(p))) < 2e-15);
}
...
BOOST_DATA_TEST_CASE(median_test, bdata::xrange(99), xr1) {
     ...
     BOOST_CHECK(std::abs(SHG::median(x) - m) < 1e-15);
}
...
BOOST_AUTO_TEST_CASE(weighted_median_test) {
     ...
     x[0] = 0;
     w[0] = 1;
     BOOST_CHECK(std::abs(weighted_median(x, w) - x[0]) < 1e-15);
     ...
     w[0] = 0;
     w[1] = 1;
     BOOST_CHECK(std::abs(weighted_median(x, w) - x[1]) < 1e-15);
     ...
     w[1] = 0;
     w[2] = 1;
     BOOST_CHECK(std::abs(weighted_median(x, w) - x[2]) < 1e-15);
     w[0] = 1;
     w[1] = 1;
     w[2] = 0;
     BOOST_CHECK(std::abs(weighted_median(x, w) -
                          0.5 * (x[0] + x[1])) < 1e-15);
     w[0] = 0;
     w[1] = 1;
     w[2] = 1;
     ...
     w[2] = 0;
     w[3] = 1;
     BOOST_CHECK(std::abs(weighted_median(x, w) - x[3]) < 1e-15);
     w[0] = 1;
     w[1] = 1;
     w[2] = 0;
     w[3] = 0;
...
BOOST_AUTO_TEST_CASE(mixture_test) {
     ...
     const double curt = nu4 / SHG::sqr(nu2) - 3.0;

     SHG::Laplace_mixture m(w, mu, lambda);

     BOOST_CHECK(std::abs(m1 - m.mean()) < 1e-14);
     BOOST_CHECK(std::abs(sdev - m.sdev()) < 1e-14);
     BOOST_CHECK(std::abs(skew - m.skew()) < 1e-14);
     BOOST_CHECK(std::abs(curt - m.curt()) < 1e-14);
     ...
     for (int i = 0; i < 3; i++) {
          BOOST_CHECK(std::abs(u.pi(i) - w0(i)) < 1e-8);
          BOOST_CHECK(std::abs(u.mu(i) - mu0(i)) < 1e-8);
          BOOST_CHECK(std::abs(u.lambda(i) - lambda0(i)) < 1e-8);
     }
}
