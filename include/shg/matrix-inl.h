/* matrix-inl.h: implementation of matrix class and functions */

/**
 * \file include/shg/matrix-inl.h
 * Implementation of matrix class and functions.
...
template <class T>
T* Matrix<T>::c_vec() {return v_.c_vec();}

template <class T>
const T* Matrix<T>::c_vec() const {return v_.c_vec();}

template <class T>
T* const* Matrix<T>::c_mat() {return p_.c_vec();}

template <class T>
T const* const* Matrix<T>::c_mat() const {return p_.c_vec();}

template <class T>
Vector<T>& Matrix<T>::vector() {return v_;}

template <class T>
const Vector<T>& Matrix<T>::vector() const {return v_;}
...
std::pair<std::pair<std::size_t, std::size_t>,
     ...
     const auto d1 = std::div(static_cast<std::ptrdiff_t>(p.first),
                              a.ncols());
     const auto d2 = std::div(static_cast<std::ptrdiff_t>(p.second),
                              a.ncols());
...
void cholesky(Matrix<T>& a, T eps) {
     ...
     // and the upper-right triangle of L contains only zeros. Put it in
     // the lower-left triangle of a, inverting the diagonal elements of
     // L.
