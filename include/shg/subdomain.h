...
struct Subdomain_error : public std::domain_error {
     Subdomain_error(Domain const& x, std::string const& description)
...
struct Subdomain {
     Subdomain(Domain const& x) : value_(x) {
...
struct Prime_predicate {
     ...
     static bool is_satisfied(T const& x) { return is_prime(x); }
     static std::string description() { return "is not prime"; }
};

template <typename T>
struct Prime : public Subdomain<T, Prime_predicate<T>> {
     Prime(T const& x) : Subdomain<T, Prime_predicate<T>>(x) {}
};
