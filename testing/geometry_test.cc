...
BOOST_AUTO_TEST_CASE(method_test) {
     ...
     line = {0.0, 1.0, 0.0};
     line.rotate(0.5 * Constants::pi<double>, {0.0, 0.0});
     BOOST_CHECK(equal(line, {1.0, 0.0, 0.0}));

     line = {0.0, 1.0, 0.0};
     line.rotate(Constants::pi<double>, {0.0, 0.0});
     BOOST_CHECK(equal(line, {0.0, 1.0, 0.0}));

     line = {0.0, 1.0, 0.0};
     line.rotate(1.5 * Constants::pi<double>, {0.0, 0.0});
     BOOST_CHECK(equal(line, {1.0, 0.0, 0.0}));

     line = {1.0, -1.0, -1.0};
     Point p1(2.0, 1.0);
     Point p2(3.0, 2.0);
     p1.rotate(Constants::pi<double> / 6.0, {3.0, 1.0});
     p2.rotate(Constants::pi<double> / 6.0, {3.0, 1.0});
     line.rotate(Constants::pi<double> / 6.0, {3.0, 1.0});
...
BOOST_AUTO_TEST_CASE(constructor_test) {
     ...
     do {
          const Rectangle r(v[0], v[1], v[2], v[3]);
...
BOOST_AUTO_TEST_CASE(verticees_test) {
     const Rectangle r{4.0, 2.0, 0.0, 0.0, 0.0};
...
BOOST_AUTO_TEST_CASE(rotate_test) {
     ...
     const double phi = Constants::pi<double> / 6.0;
     const Point c{12.0, 8.0};
     ...
     const Rectangle r1{p1, p2, p3, p4};
...
BOOST_AUTO_TEST_CASE(equal_test) {
     ...
     const double h = 0.0;
     const double k = 0.0;
     const double eps = std::ldexp(1.0, -4);
     const double delta1 = std::ldexp(1.0, -5);
     const double delta2 = std::ldexp(1.0, -3);
...
BOOST_AUTO_TEST_CASE(stream_insertion_test) {
     std::ostringstream oss;
     const Rectangle r{4.0, 2.0, 1.57, 8.0, 9.0};
     oss << std::fixed << std::setprecision(4) << r << '\n';
     BOOST_CHECK(
          oss.str() ==
...
BOOST_AUTO_TEST_CASE(trigonometric_test) {
     const Ellipse e{2.0, 1.0, 0.0, 0.0, 0.0};
     Point p;
     double t;
     ...
     t = 0.5 * Constants::pi<double>;
     p = {0.0, 1.0};
     BOOST_CHECK(equal(e.trigonometric(t), p, tolerance));

     t = Constants::pi<double>;
     p = {-2.0, 0.0};
     BOOST_CHECK(equal(e.trigonometric(t), p, tolerance));

     t = 1.5 * Constants::pi<double>;
     p = {0.0, -1.0};
     BOOST_CHECK(equal(e.trigonometric(t), p, tolerance));
}
...
BOOST_AUTO_TEST_CASE(tangent_test) {
     const Ellipse e{4.0, 2.0, 0.0, 0.0, 0.0};
     Point p;
     Line result;
     Line line;
...
BOOST_AUTO_TEST_CASE(rotate_test) {
     Ellipse e(4.0, 2.0, 0.0, 0.0, 0.0);
     e.rotate(Constants::pi<double> / 6.0, {0.0, 0.0});
     BOOST_CHECK(faeq(e.a(), 4.0, tolerance));
     BOOST_CHECK(faeq(e.b(), 2.0, tolerance));
     BOOST_CHECK(
          faeq(e.phi(), Constants::pi<double> / 6.0, tolerance));
...
BOOST_AUTO_TEST_CASE(stream_insertion_test) {
     std::ostringstream oss;
     const Ellipse e{4.0, 2.0, 1.57, 8.0, 9.0};
...
std::ostream& operator<<(std::ostream& stream,
                         const Common_points_test_case& c) {
     stream.width(0);
     stream << c.e1 << ", " << c.e2 << '\n';
     return stream;
}

const std::vector<Common_points_test_case> cptc{
     ...
     {{4.0, 2.0, 0.0, 0.0, 0.0},
      {2.0, 1.0, 0.5 * Constants::pi<double>, 0.0, 0.0},
      0,
      {{0.0, -2.0}, {0.0, 2.0}}},
     ...
     // intersection
     {{4.0, 2.0, 0.0, 0.0, 0.0},
      {4.0, 2.0, 0.5 * Constants::pi<double>, 0.0, -2.0},
      0,
     ...
     {{4.0, 2.0, 0.0, 0.0, 0.0},
      {4.0, 2.0, 0.5 * Constants::pi<double>, 0.0, 0.0},
      0,
...
BOOST_DATA_TEST_CASE(common_points_test, bdata::make(cptc), c) {
     ...
     Vecpoint p;
     const int r = common_points(c.e1, c.e2, p);
     BOOST_CHECK(r == c.result);

     if (equal(c.e2,
               Ellipse(4.0, 2.0, 0.5 * Constants::pi<double>, 0.0,
                       -2.0),
               tolerance)) {
          /*
...
BOOST_AUTO_TEST_CASE(distance_from_point_to_ellipse_test) {
     ...
     const Point p0{2.0, std::sqrt(3.0)};
