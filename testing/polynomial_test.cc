...
BOOST_DATA_TEST_CASE(polynomial_div_test,
     ...
     const auto& c = pdtcase[xr];
     const auto s = c.g.size();
...
struct Test_data {
     ...
     void init(const std::vector<const char*> f,
               const std::vector<const char*> g,
               const char* ordering);
};

void Test_data::init(const std::vector<const char*> f,
                     const std::vector<const char*> g,
                     const char* ordering) {
     Monomial_cmp cmp;
...
BOOST_DATA_TEST_CASE(buchberger_test,
     ...
     const auto& tc = buchberger_test_data[xr];
     ...
     b.run(td.f_);
     BOOST_CHECK(have_equal_content(b.g(), td.g_));
     BOOST_CHECK(true);
}
