...
BOOST_AUTO_TEST_CASE(all_dates_test) {
     ...
     for (int y = Date::min().year(); y <= Date::max().year(); y++) {
          for (int m = 1; m <= 12; m++) {
               int const last = Date::lastdom(m, y);
               c = 0;
               for (int d = 1; d <= last; d++) {
                    Date const dt(d, m, y);
                    int const dow = dt.dayofweek();
                    c[dow]++;
...
constexpr char const* const incorrect_input[] = {
     "02 stycznia 1901", "02 I 1901", "2.1.1901", "02.1.1901",
     "2.01.1901"};

constexpr char const* const correct_input[] = {
...
public:
     ...
     struct iterator {
          ...
          static Date const first1;
          static Date const last1;
          static Date const first2;
          static Date const last2;
          static Date const first3;
          static Date const last3;
...
Date const Date_dataset::iterator::first1 = Date::min();
Date const Date_dataset::iterator::last1 = Date::min() + 40000;
Date const Date_dataset::iterator::first2 = Date(1, Date::jan, 1899);
Date const Date_dataset::iterator::last2 = Date(31, Date::dec, 2400);
Date const Date_dataset::iterator::first3 = Date::max() - 40000;
Date const Date_dataset::iterator::last3 = Date::max();
