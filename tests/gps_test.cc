...
using SHG::GPS::Geogr_coord;
using SHG::GPS::Cart_coord;
using SHG::GPS::semi_major_axis;
using SHG::GPS::semi_minor_axis;
using SHG::GPS::GPX_data;
using SHG::GPS::Activity_statistics;
using SHG::facmp;
...
struct AS_data {
     ...
     bool const should_be_ungzipped;
};

AS_data const as_data[] = {
     {TESTS::GPX::gpx_gz_5474850155,
      TESTS::GPX::gpx_gz_5474850155_len, "5474850155.gpx.gz", true},
     {TESTS::GPX::gpx_gz_5480723783,
      TESTS::GPX::gpx_gz_5480723783_len, "5480723783.gpx.gz", true},
     {TESTS::GPX::gpx_gz_5733570778,
      TESTS::GPX::gpx_gz_5733570778_len, "5733570778.gpx.gz", false},
     {TESTS::GPX::gpx_gz_afternoon_ride,
      TESTS::GPX::gpx_gz_afternoon_ride_len, "Afternoon_Ride.gpx.gz",
      true},
};
...
void setup() {
     ...
     for (std::size_t i = 0; i < std::size(as_data); i++) {
          ...
          std::istringstream iss(std::string(sp, a.size), bininp);
...
Activity_statistics::Result const res[] = {
...
BOOST_AUTO_TEST_CASE(activity_statistics_test,
                     *boost::unit_test::fixture(&setup, &teardown)) {
     namespace fs = std::filesystem;
