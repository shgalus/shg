...
using SHG::ALGEBRA::Recursive_monomial_generator;
using SHG::ALGEBRA::Iterative_monomial_generator;
...
BOOST_AUTO_TEST_CASE(monomial_test) {
     ...
     for (int n = 1; n <= 5; n++) {
          ...
          for (int i = 0; i < n; i++)
               d.push_back(i + 1);
          int const td = std::accumulate(d.cbegin(), d.cend(), 0);
          Monomial m{d};
          BOOST_CHECK(m.dim() == n);
...
constexpr char const* const result = R"(
 000
 001 010 100
...
BOOST_AUTO_TEST_CASE(recursive_monomial_generator_test) {
     ...
     auto fun = [&buf](std::vector<int> const& a) {
          buf << ' ';
