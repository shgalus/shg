...
using SHG::degree_of_polynomial;
using SHG::Minimizer;
using SHG::Uniform_search_for_minimum;
using SHG::Multimin_fminimizer;
using SHG::Vector;
using SHG::Vecdouble;
using SHG::Veccomplex;
using SHG::Matrix;
...
BOOST_AUTO_TEST_CASE(solve_linear_test) {
     ...
     Vecdouble const x{-4.05205, -12.6056, 1.66091, 8.69377};
...
BOOST_AUTO_TEST_CASE(solve_linear_throws_test) {
     ...
     Vecdouble x{1.0};
     auto is_correct = [](std::runtime_error const& e) {
          return std::strcmp(
                      e.what(),
...
BOOST_AUTO_TEST_CASE(minimizer_test) {
     ...
     BOOST_CHECK(faeq(m.x_minimum(), pi<double>, 1e-8));
...
BOOST_AUTO_TEST_CASE(Uniform_search_for_minimum_test) {
     ...
     BOOST_CHECK_THROW(s.search(f1, 1.0, -1.0, 0.01),
                       std::invalid_argument);
     double const eps = 3.0 / std::numeric_limits<int>::max();
...
double my_f(gsl_vector const* v, void* params) {
     double x, y;
