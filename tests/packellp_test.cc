...
BOOST_DATA_TEST_CASE(min_alpha_rectangle_test, bdata::xrange(36),
     ...
     std::vector<Ellipse> v = {
          {4.0, 2.0, 0.0, 0.0, 0.0},
          {2.0, 1.0, 0.5 * pi<double>, 5.0, 0.0}};
     Rectangle r{10.0, 4.0, 0.0, 1.0, 0.0};
     double const phi = SHG::degrees_to_radians(5.0 * xr);
     double const alpha = xr < 18 ? phi : phi - 0.5 * pi<double>;
     Point const p{10.0, 10.0};
...
BOOST_DATA_TEST_CASE(is_feasible_packing_test, bdata::xrange(21),
     ...
     std::vector<Ellipse> v = {
          {4.0, 2.0, 0.0, 0.0, 0.0},
          {2.0, 1.0, 0.5 * pi<double>, 4.9 + xr * 0.01, 0.0}};
...
BOOST_AUTO_TEST_CASE(min_rectangle_for_semiaxes_test) {
     ...
     std::vector<Semiaxes> const semiaxes{{4.0, 2.0}, {2.0, 1.0}};
...
BOOST_AUTO_TEST_CASE(min_rectangle_for_semiaxes_3_ellipses_test) {
     ...
     Rectangle const r0{1.6149905e+01, 4.0000053e+00, 0.0000000e+00,
                        8.0749527e+00, -2.6463902e-06};
     std::vector<Ellipse> const e0{
...
std::ostream& operator<<(std::ostream& stream,
                         Congruent_regular_test_case const& c) {
     stream.width(0);
     stream << c.n << '\n';
     return stream;
}

std::vector<Congruent_regular_test_case> const crtc{
