...
double pdf(int n1, int n2, int k) {
     if (k % 2 == 0) {
          ...
          int const l = (k - 1) / 2;
          double const s1 = newton(n1 - 1, l - 1) * newton(n2 - 1, l);
          double const s2 = newton(n1 - 1, l) * newton(n2 - 1, l - 1);
          double const s3 = newton(n1 + n2, n1);
          return (s1 + s2) / s3;
     }
}
