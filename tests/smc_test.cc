...
BOOST_AUTO_TEST_CASE(smc_test) {
     ...
     Vector<Result> const ex1xsj{
          {1, 0, 2}, {4, 1, 3}, {3, 5, 4}, {1, 8, 0}};
     Vecint const ex1z{2, 3, 3, 3, 3, 4, 4, 4, 0};
     Vector<Result> const ex2xsj{
          {1, 0, 2}, {4, 1, 3}, {3, 5, 4}, {2, 8, 0}};
     Vecint const ex2z{2, 3, 3, 3, 3, 4, 4, 4, 0, 0};
...
BOOST_AUTO_TEST_CASE(unideggaumix_test) {
     ...
     int k = 0;
     for (int i = 0; i < 4; i++) {
          int const n = iceil(u.n * u.pi(i));
          double const mu = u.mu(i);
          double const sigma = u.sigma(i);
          for (int j = 0; j < n; j++)
     ...
     Vecdouble const p{1.000090989306e-01, 2.005154391418e-01,
                       2.991848410989e-01, 2.002906213288e-01,
                       1.999999994999e-01};

     Vecdouble const mu{-8.047728616669e-02, -1.016764381500e-02,
                        1.016802041740e-02, 8.021842431891e-02};

     Vecdouble const sigma{1.918811543256e-02, 5.006600877334e-03,
                           4.972330263071e-03, 2.086705495178e-02};

     Vecdouble::size_type const K = u.K;
     Vecdouble::size_type const K1 = u.K1;
